...
# Static values used to select activation function (belive it will be sigmoid that is the best)
acts = {
...
# Static values used to select optimizer (belive it will be adam that is the best, we do not have to adjust hyperparameters) 
optimizers = {
...
class NeuralNet:
    ...
    def init_model(self, nn_dims, board_size, lr, activation, optimizer):
        ...
        if activation_function_actor is None:
            raise ValueError(
        ...
        model = ks.Model(inputs=x, outputs=[actor_output])
        losses = {
